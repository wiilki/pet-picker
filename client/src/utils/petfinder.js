const petfinder = {
    ...
    search: async function(searchOptions) {
      ...
      const data = await response.json();
      const filteredData = data.animals.filter(pet => pet.photos.length > 0); // filter out pets without images
      return { ...data, animals: filteredData }; // return filtered data with the same structure as the original data
    }
